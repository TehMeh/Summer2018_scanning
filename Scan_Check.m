...
<<"<path to MadBounds.m>";
pathmg= "< Path to /MG5_aMC_v2_6_3_2 >";
pathhb= "< Path to HiggsBounds >";
pathin="<Working directory>";
...
FindHiggsRelation[ err_]:=Module[
	...
	While[  If[out2==={} || out2==={invalid}, True,  
		...
		mh=125.09;(*RandomReal[{50,800}];*)
		mH=RandomReal[{180,1200}];
		mA=RandomReal[{180,1200}];
		teta12=RandomReal[{0,2*\[Pi]}];
		s12=Sin[teta12];
		c12=Cos[teta12];
		s13=0;
		mZ=91.1876;
	...
	If[direction===Table[0.,{j,8}],(*Print["Did not find a valid direction!"];*)
			exitStatus=1; out2={invalid},
			...
			While[ If[mhSM<mhcurr,
					If[mhcurr<=mhprev,True, False], 
						If[mhcurr<=mhprev,False, True]  ] && 
...
derivatives=Flatten[#[[1]]&/@idata,1]; (*separate derivatives*)
idatanew=#[[2]]&/@idata; (*separate GN data*)


idata2=DeleteCases[idatanew,{invalid}] (* delete failed points*)
idata3=DeleteCases[If[#[[2,2,2]]=!=0. && #[[2,2,1]]=!=0., #]&/@idata2, Null]; (*delete entries with non-scalar lightest Higgs particles*)
idata4={Flatten[#[[1]]],#[[2]],#[[3]],#[[4]]}&/@idata3; (*Flatten the list containing masses*)
idata5=Swapper[idata4]; (*swap heavy scalar and pseudoscalar Higgs particles if needed*)
idata6=DeleteCases[If[Abs[#[[1,2]]-125.09]<0.32, #]&/@idata5,Null]; (*delete entries with bad lightest Higgs masses*)
(*idata7=ZMassConstraints[idata6]; (*Apply potential constraints*)  *)
...
Export[NotebookDirectory[]<>"derivatives.txt",derivatives,"List"]
Export[NotebookDirectory[]<>"Gradient.txt",final,"List"]
