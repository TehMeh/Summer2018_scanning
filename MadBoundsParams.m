...
BeginPackage["MadBoundsParams`"]
ParamBlocks::usage="Contains parameter names";
SMParamBlocks::usage="Contains SM parameter names";
...
BoundsParams[np_,cp_]:={
	...
	{"BR_Hplus.dat",
		Flatten[
			{Table[BRHpcs[[j]], {j,cp}], Table[BRHpcb[[j]], {j,cp}], Table[BRHptaunu[[j]], {j,cp}]}
		]
	},
...
Begin["Private`"]
	...
	fermionmass={lst[[1,18]],lst[[1,15]],lst[[1,19]],
				lst[[1,16]],lst[[1,8]],lst[[1,9]] 
		};
	...
	finalrep={
		...
		Mh->tMh,
		Mhplus -> tMhplus
		(*LepHmHp production rates are 0 by default and not included*)
	}	
];
End[]
